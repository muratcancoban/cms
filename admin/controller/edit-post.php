...
// etiketler
$tags = $db->from('post_tags')//var olan etiketler
    ->join('tags', '%s.tag_id = %s.tag_id')//tags tablosunda tag_id si mevcut tag idye eşit olanı buluyoruz
...
if (post('submit')){
    ...
    if (!$post_url || !$post_content || !$post_status || !$post_categories ){
        ...
        if ($query){
            ...
            $query = $db->update('posts')//yenilenen post_id ye eşit olan id güncellenecek
                ->where('post_id', $id)
                ->set([
            ...
            if ($query){
                ...
                $diffs = array_diff($postTags, $post_tags);//farklılıkları alıyoruz
                if (count($diffs) > 0){
                    foreach ($diffs as $diff){//farklı olanları silmek için fönüye alıyoruz
                        foreach ($allTags as $allTag){//bütün listelenen tagleri alıp
                            if (trim($allTag['tag_name']) == $diff){
                                $db->delete('post_tags')//post_tags tablosundan tag_id'si alltag ile eşlenen tagi siliyoruz
                                    ->where('tag_id', $allTag['tag_id'])
                                    ->where('tag_post_id', $id)//bu tagid'si ve tag_post_id şu anki düzenlenen posta ait olanı silecek
                                    ->done();
                            }
                        }
                    }
