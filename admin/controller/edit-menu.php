...
$id = get('id');
if (!$id){//eğer id kısmı yoksa menü kısmına geri döndürecek
...
if (post('submit')) {
    ...
    if (!$menu_title) {
        ...
        $query = $db->prepare('UPDATE menu SET menu_title = :menu_title, menu_content = :menu_content WHERE menu_id = :id');//güncelleme işlemi yapılıyor
...
$menuData = json_decode($row['menu_content'], true);//menü content altında kaydettiğimiz jasonı tekrardan diziye aktardık
