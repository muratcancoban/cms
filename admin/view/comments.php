    ...
    <div class="filter">
        <ul>
            ...
            <li class="<?=get('status') == 2 ? 'active' : null?>">
                <a href="<?=admin_url('comments?status=2')//class="btn"?>">
                    Onay Bekleyenler
                </a>
            </li>
        </ul>
                ...
                <tr>
                    ...
                    <td>
                        <p style="font-size: 12px; color: #666;">
                            ...
                            <?php if ($row['comment_status'] == 0): //statü 0'ise?>
...
<?php if ($totalRecord > $pageLimit): ?>
    <div class="pagination">
        <ul>
            <?= $db->showPagination(admin_url(route(1) . '?' . $pageParam . '=[page]&status=' . get('status'))) //status varsa yazacak?>
        </ul>
    </div>
<?php endif; ?>
